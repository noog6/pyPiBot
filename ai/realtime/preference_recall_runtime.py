...
def _emit_preference_recall_skip_trace_if_needed(controller, *, turn_id: str | None) -> None:
    ...
    recall_invoked = any(
        ...
        and str(record.get("turn_id") or "").strip() in {"", resolved_turn_id}
...
async def _maybe_handle_preference_recall_intent(controller, text: str, websocket: Any, *, source: str) -> bool:
    ...
    if not input_event_key:
        input_event_key = controller._active_input_event_key_for_turn(resolved_turn_id)
    ...
    try:
        ...
        if cached and cooldown_s > 0.0 and now - float(cached.get("timestamp", 0.0)) < cooldown_s:
        ...
        await controller._attach_preference_recall_result_to_response(
            websocket=websocket,
            turn_id=resolved_turn_id,
            input_event_key=input_event_key,
            query=query,
