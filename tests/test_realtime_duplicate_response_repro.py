...
def test_server_auto_rebind_cancels_transcript_watchdog() -> None:
    api = _make_api_stub()
    api._transcript_response_watchdog_timeout_s = 0.05
    api._active_response_origin = "server_auto"
    api._response_in_flight = True
    api._active_server_auto_input_event_key = "synthetic_server_auto_1"

    micro_ack_calls: list[dict[str, object]] = []
    api._maybe_schedule_micro_ack = lambda **kwargs: micro_ack_calls.append(kwargs)

    async def _run() -> None:
        api._start_transcript_response_watchdog(turn_id="turn_1", input_event_key="item_pants")
        watchdog_task = api._transcript_response_watchdog_tasks.get("item_pants")
        assert watchdog_task is not None

        api._rebind_active_response_correlation_key(
            turn_id="turn_1",
            replacement_input_event_key="item_pants",
        )

        # Mirror post-response timing where playback can still be busy when the watchdog timeout hits.
        api._response_in_flight = False
        api._audio_playback_busy = True
