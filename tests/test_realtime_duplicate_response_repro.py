...
def test_service_motion_followup_tagged_when_inspect_tool_result_exists_for_turn() -> None:
    api = _make_api_stub()
    _wire_runtime(api)
    api._current_response_turn_id = "turn_service_motion"
    api._active_input_event_key_by_turn_id["turn_service_motion"] = "item_parent_service_motion"
    api._tool_call_records = [{"turn_id": "turn_service_motion", "name": "inspect_current_view"}]

    response_create_event, _ = api._build_tool_followup_response_create_event(
        call_id="call_service_motion",
        response_create_event={"type": "response.create"},
        tool_name="gesture_look_center",
    )

    metadata = ((response_create_event.get("response") or {}).get("metadata") or {})
    assert metadata.get("service_motion_candidate") == "true"
    assert metadata.get("service_motion_for_visual_actuator") == "explicit_inspect"
    assert metadata.get("service_motion_semantic_owner") == "inspect_current_view"


def test_service_motion_followup_suppressed_when_explicit_inspect_present() -> None:
    api = _make_api_stub()
    _wire_runtime(api)
    api._tool_call_records = [{"turn_id": "turn_service_motion", "name": "inspect_current_view"}]
    ...
    should_drop, _parent_entry, reason = api._should_suppress_queued_tool_followup_release(
        response_metadata={
            "turn_id": "turn_service_motion",
            "tool_name": "gesture_look_center",
            "service_motion_candidate": "true",
        },
        blocked_by_response_id=None,
    )

    assert should_drop is True
    assert reason == "service_motion_for_explicit_inspect"


def test_non_inspect_gesture_followup_path_unchanged_without_inspect_result() -> None:
    api = _make_api_stub()
    _wire_runtime(api)

    should_drop, _parent_entry, reason = api._should_suppress_queued_tool_followup_release(
        response_metadata={
            "turn_id": "turn_regular_gesture",
            "tool_name": "gesture_look_center",
            "service_motion_candidate": "true",
        },
        blocked_by_response_id=None,
