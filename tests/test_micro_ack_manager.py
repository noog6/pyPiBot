...
def test_micro_ack_schedules_and_emits() -> None:
    ...
    logs: list[tuple[str, str, str, int | None, str | None, str | None, str | None, str | None, str | None, str | None, str | None]] = []
    now = 0.0
    ...
    manager = MicroAckManager(
        ...
        on_log=lambda event, turn_id, reason, delay_ms, category, channel, intent, action, tool_call_id, dedupe_fingerprint, suppression_source: logs.append((event, turn_id, reason, delay_ms, category, channel, intent, action, tool_call_id, dedupe_fingerprint, suppression_source)),
    ...
    assert any(
        event == "scheduled"
        and isinstance(dedupe_fingerprint, str)
        and len(dedupe_fingerprint) == 8
        for event, _turn_id, _reason, _delay, _category, _channel, _intent, _action, _tool_call_id, dedupe_fingerprint, _suppression_source in logs
    )
    assert any(
        event == "emitted"
        and category == "speech"
        ...
        and isinstance(dedupe_fingerprint, str)
        and len(dedupe_fingerprint) == 8
        for event, _turn_id, _reason, _delay, category, channel, _intent, _action, _tool_call_id, dedupe_fingerprint, _suppression_source in logs
    )
    loop.close()
...
def test_micro_ack_suppresses_for_recent_talk_over() -> None:
    ...
    assert any(
        event == "suppressed"
        and turn_id == "turn-2"
        and reason == "talk_over_risk"
        and category == "speech"
        and channel == "voice"
        and isinstance(dedupe_fingerprint, str)
        and len(dedupe_fingerprint) == 8
        and suppression_source == "baseline"
        for event, turn_id, reason, _delay, category, channel, _intent, _action, _tool_call_id, dedupe_fingerprint, suppression_source in logs
    )
    loop.close()
...
def test_channel_disabled_logs_single_suppression_without_scheduling_state_change() -> None:
    ...
    assert len(logs) == 1
    event, turn_id, reason, delay, category, channel, intent, action, tool_call_id, dedupe_fingerprint, suppression_source = logs[0]
    assert (event, turn_id, reason, delay, category, channel, intent, action, tool_call_id) == (
        "suppressed",
        "turn-channel-disabled",
        "channel_disabled",
        None,
        "speech",
        "voice",
    ...
    assert isinstance(dedupe_fingerprint, str) and len(dedupe_fingerprint) == 8
    assert suppression_source is None
...
def test_same_category_intent_within_ttl_is_suppressed() -> None:
    ...
    assert len(emits) == 1
    assert any(event == "suppressed" and turn_id == "turn-1" and reason == "duplicate_within_ttl" and category == "speech" and channel == "voice" for event, turn_id, reason, _delay, category, channel, _intent, _action, _tool_call_id, dedupe_fingerprint, suppression_source in logs)
    loop.close()
...
def test_global_cooldown_scope_all_blocks_voice_then_text() -> None:
    ...
    assert any(event == "suppressed" and reason == "cooldown" and channel == "text" for event, _turn, reason, _delay, _category, channel, _intent, _action, _tool_call_id, dedupe_fingerprint, suppression_source in logs)
    loop.close()
...
def test_per_channel_cooldown_blocks_same_channel_only() -> None:
    ...
    assert any(
        event == "suppressed"
        and reason == "channel_cooldown"
        and category == "watchdog"
        and channel == "voice"
        and isinstance(dedupe_fingerprint, str)
        and len(dedupe_fingerprint) == 8
        and suppression_source == "cooldown"
        for event, _turn, reason, _delay, category, channel, _intent, _action, _tool_call_id, dedupe_fingerprint, suppression_source in logs
    )
    loop.close()
...
def test_per_category_cooldown_blocks_same_category_only() -> None:
    ...
    assert any(
        ...
        for event, _turn, reason, _delay, category, _channel, _intent, _action, _tool_call_id, _dedupe_fingerprint, _suppression_source in logs
    )
    loop.close()
