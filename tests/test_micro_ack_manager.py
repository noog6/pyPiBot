...
def test_micro_ack_schedules_and_emits() -> None:
    ...
    manager = MicroAckManager(
        ...
        on_emit=lambda turn_id, phrase_id, phrase: emits.append((turn_id, phrase_id, phrase)),
    ...
    manager.maybe_schedule(turn_id="turn-1", reason="speech_stopped", loop=loop, expected_delay_ms=900)
    loop.run_until_complete(asyncio.sleep(0.03))

    assert emits and emits[0][0] == "turn-1"
...
def test_micro_ack_suppresses_for_recent_talk_over() -> None:
    ...
    manager.maybe_schedule(turn_id="turn-2", reason="speech_stopped", loop=loop, expected_delay_ms=900)
