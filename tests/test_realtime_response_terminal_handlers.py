...
def test_handle_response_done_skips_redrive_when_required_deliverable_retry_budget_exhausted() -> None:
    api = _make_api()
    api._active_response_origin = "tool_output"
    api._response_done_deliverable_arbitration = Mock(
        return_value=TerminalDeliverableDecision(
    ...
    api._should_admit_response_terminal_event = lambda **_kwargs: True
    api._consume_required_deliverable_materialization_retry_budget = Mock(return_value=(False, 2, 2))
    api._release_required_deliverable_followthrough_dispatch_lock = Mock()
    api._dispatch_required_deliverable_followthrough_response_create = AsyncMock(return_value=True)
    api._maybe_schedule_empty_response_retry = AsyncMock()
    api._build_confirmation_transition_decision = Mock(
...
def test_handle_response_done_skips_redrive_when_required_deliverable_materialization_already_pending() -> None:
    api = _make_api()
    api._active_response_origin = "tool_output"
    api._response_done_deliverable_arbitration = Mock(
        return_value=TerminalDeliverableDecision(
    ...
    api._has_required_deliverable_followthrough_materialization_pending = Mock(return_value=True)
    api._should_admit_response_terminal_event = lambda **_kwargs: True
    api._consume_required_deliverable_materialization_retry_budget = Mock(return_value=(True, 1, 2))
    api._release_required_deliverable_followthrough_dispatch_lock = Mock()
    api._dispatch_required_deliverable_followthrough_response_create = AsyncMock(return_value=True)
    ...
    api._consume_required_deliverable_materialization_retry_budget.assert_not_called()
    api._release_required_deliverable_followthrough_dispatch_lock.assert_not_called()
    api._dispatch_required_deliverable_followthrough_response_create.assert_not_awaited()
...
def test_handle_response_done_required_deliverable_missing_substance_keeps_followthrough_open() -> None:
    api = _make_api()
    api._active_response_origin = "tool_output"
    ...
    api._active_response_id = "resp_tool_required_empty"
    api._active_input_event_key_for_turn = lambda _turn_id: "item_parent"
    api._is_empty_response_done = lambda **_kwargs: False
    api._response_trace_context_by_id = {
        "resp_tool_required_empty": {
    ...
    asyncio.run(
        api.handle_response_done(
            {"type": "response.done", "response": {"id": "resp_tool_required_empty"}}
        )
    )

    apply_selection.assert_called_once_with(
        canonical_key="turn_1::tool:call_final_report",
        semantic_owner_canonical_key="turn_1::item_parent",
...
def test_handle_response_done_required_deliverable_rejects_progress_only_terminal_text() -> None:
    api = _make_api()
    api._active_response_origin = "tool_output"
    api._active_response_input_event_key = "tool:call_final_report"
    api._active_response_canonical_key = "turn_1::tool:call_final_report"
    api._active_response_id = "resp_tool_required_progress_only"
    api._active_input_event_key_for_turn = lambda _turn_id: "item_parent"
    api._is_empty_response_done = lambda **_kwargs: False
    api._response_trace_context_by_id = {
        "resp_tool_required_progress_only": {
            "tool_followup_step_output_policy": "required_deliverable",
            "tool_followup_post_completion_reason": "required_deliverable_owed",
            "followthrough_step_output_policy": "required_deliverable",
            "followthrough_post_completion_reason": "required_deliverable_owed",
    ...
    api._terminal_response_text_by_response_id = {
        "resp_tool_required_progress_only": "I'm back to center now. Let me check my voltage for you.",
    }
    ...
    asyncio.run(
        api.handle_response_done(
            {"type": "response.done", "response": {"id": "resp_tool_required_progress_only"}}
        )
    )
...
def test_handle_response_done_required_deliverable_missing_tool_execution_keeps_followthrough_open() -> None:
    api = _make_api()
    api._active_response_origin = "tool_output"
    api._active_response_input_event_key = "tool:call_final_report"
    api._active_response_canonical_key = "turn_1::tool:call_final_report"
    api._active_response_id = "resp_tool_required_bridge_only"
    api._active_input_event_key_for_turn = lambda _turn_id: "item_parent"
    api._is_empty_response_done = lambda **_kwargs: False
    api._response_trace_context_by_id = {
        "resp_tool_required_bridge_only": {
            "tool_followup_step_output_policy": "required_deliverable",
            "tool_followup_post_completion_reason": "required_deliverable_owed",
            "followthrough_step_output_policy": "required_deliverable",
            "followthrough_post_completion_reason": "required_deliverable_owed",
    ...
    api._selected_response_has_terminal_text_evidence = lambda **_kwargs: True
    api._required_deliverable_tool_execution_missing = lambda **_kwargs: True
    api._response_done_followthrough_chain_remaining = lambda **_kwargs: False
    api._maybe_schedule_empty_response_retry = AsyncMock()
    api._build_confirmation_transition_decision = Mock(
        return_value=SimpleNamespace(
    ...
    asyncio.run(
        api.handle_response_done(
            {"type": "response.done", "response": {"id": "resp_tool_required_bridge_only"}}
        )
    )

    apply_selection.assert_called_once()
    assert apply_selection.call_args.kwargs["selected"] is False
    assert apply_selection.call_args.kwargs["selection_reason"] == "required_deliverable_missing_tool_execution"
    apply_continuity_event.assert_called_once()
    _event_name, kwargs = apply_continuity_event.call_args
    assert kwargs["keep_ongoing"] == "true"
    assert kwargs["complete_required_deliverable"] == ""
...
def test_handle_response_done_server_auto_cannot_win_terminal_selection_while_required_deliverable_pending() -> None:
    api = _make_api()
    api._active_response_origin = "server_auto"
    api._active_response_input_event_key = "item_1"
    api._active_response_canonical_key = "turn_1::item_1"
    api._active_response_id = "resp_server_auto_intermediate"
    api._response_trace_context_by_id = {"resp_server_auto_intermediate": {}}
    ...
    asyncio.run(
        api.handle_response_done(
            {"type": "response.done", "response": {"id": "resp_server_auto_intermediate"}}
        )
    )
...
def test_handle_response_done_required_deliverable_followthrough_with_terminal_text_completes_contract() -> None:
    ...
    api._active_response_input_event_key = "tool:call_final_report"
    api._active_response_canonical_key = "turn_1::tool:call_final_report"
    api._active_response_id = "resp_tool_required_complete"
    api._active_input_event_key_for_turn = lambda _turn_id: "item_parent"
    api._is_empty_response_done = lambda **_kwargs: False
    api._response_trace_context_by_id = {
        "resp_tool_required_complete": {
            "tool_followup_step_output_policy": "required_deliverable",
            "tool_followup_post_completion_reason": "required_deliverable_owed",
            "followthrough_step_output_policy": "required_deliverable",
            "followthrough_post_completion_reason": "required_deliverable_owed",
    ...
    asyncio.run(
        api.handle_response_done(
            {"type": "response.done", "response": {"id": "resp_tool_required_complete"}}
        )
    )
...
def test_handle_response_done_bridge_skips_empty_retry_when_status_only_followup_pending() -> None:
    ...
    def _pending_followup(*, include_status_only: bool = False, **_kwargs: object) -> bool:
        return include_status_only

    api._turn_has_pending_tool_followup = _pending_followup
    api._turn_followthrough_chain_remaining = lambda **_kwargs: True
    api._maybe_schedule_empty_response_retry = AsyncMock()
    api._build_confirmation_transition_decision = Mock(
        return_value=SimpleNamespace(
...
def test_handle_response_done_parent_promoted_cross_turn_final_tool_report_clears_parent_followthrough() -> None:
    api = _make_api()
    ...
    parent_brief = api.get_continuity_brief("run-test", "turn_2", reason="post_cross_turn_parent_promoted_tool_report")
    assert parent_brief.compound_request is None
    assert parent_brief.commitments == ()


def test_handle_response_done_parent_promoted_followthrough_guard_uses_parent_turn_for_final_close() -> None:
    api = _make_api()
    api._current_turn_id_or_unknown = lambda: "turn_3"
    api._active_input_event_key_for_turn = lambda turn_id: "item_parent" if turn_id == "turn_2" else "tool:call_final"
    api._apply_continuity_event(
    ...
    assert checked_turn_ids[-1] == "turn_2"
    response_done_calls = [call for call in apply_continuity_event.call_args_list if call.args and call.args[0] == "response_done"]
    assert len(response_done_calls) == 1
    _, kwargs = response_done_calls[0]
    assert kwargs["turn_id"] == "turn_2"
...
def test_handle_response_done_parent_promoted_intermediate_tool_output_does_not_clear_final_report() -> None:
    api = _make_api()
    api._current_turn_id_or_unknown = lambda: "turn_3"
    api._active_input_event_key_for_turn = lambda turn_id: "item_parent" if turn_id == "turn_2" else "tool:call_middle"
    api._apply_continuity_event(
        "transcript_final",
        text="Look right, check diagnostics, then report done.",
        source="input_audio_transcription",
    ...
    api._active_response_origin = "tool_output"
    api._active_response_input_event_key = "tool:call_middle"
    api._active_response_canonical_key = "turn_3::tool:call_middle"
    api._active_response_id = "resp_tool_middle"
    api._response_done_followthrough_chain_remaining = lambda **_kwargs: True
...
def test_handle_response_done_cross_turn_empty_tool_output_does_not_complete_parent_report() -> None:
    api = _make_api()
    api._current_turn_id_or_unknown = lambda: "turn_3"
    api._active_input_event_key_for_turn = lambda turn_id: "item_parent" if turn_id == "turn_2" else "tool:call_final"
    api._apply_continuity_event(
    ...
    api._active_response_input_event_key = "tool:call_final"
    api._active_response_canonical_key = "turn_3::tool:call_final"
    api._active_response_id = "resp_tool_final_empty"
    api._response_done_followthrough_chain_remaining = lambda **_kwargs: False
    api._is_empty_response_done = lambda **_kwargs: True
    api._semantic_owner_decision_for_response = Mock(
        return_value=SimpleNamespace(
    ...
    asyncio.run(api.handle_response_done({"type": "response.done", "response": {"id": "resp_tool_final_empty"}}))

    response_done_calls = [call for call in apply_continuity_event.call_args_list if call.args and call.args[0] == "response_done"]
    assert len(response_done_calls) == 1
    _, kwargs = response_done_calls[0]
    assert kwargs["complete_final_report"] == ""
    parent_brief = api.get_continuity_brief("run-test", "turn_2", reason="post_cross_turn_empty_tool_output")
    assert parent_brief.compound_request is not None
    assert any(step.kind == "report" and step.status == "pending" for step in parent_brief.compound_request.steps)
...
def test_handle_response_done_wrong_lineage_final_answer_does_not_clear_parent_compound_state() -> None:
    ...
    api._active_response_input_event_key = "tool:call_final"
    api._active_response_canonical_key = "turn_3::tool:call_final"
    api._active_response_id = "resp_tool_final_wrong_lineage"
    api._response_done_followthrough_chain_remaining = lambda **_kwargs: False
    api._terminal_response_text_by_response_id = {"resp_tool_final_wrong_lineage": "Final answer from unrelated lineage."}
    api._semantic_owner_decision_for_response = Mock(
        return_value=SimpleNamespace(
            semantic_owner_canonical_key="turn_3::tool:call_final",
            parent_turn_id="turn_2",
            parent_input_event_key="item_parent",
            selected_candidate_id="semantic_owner_execution",
            reason_code="tool_output_owner_retained",
        )
    )
    api._maybe_schedule_empty_response_retry = AsyncMock()
    api._build_confirmation_transition_decision = Mock(
    ...
    asyncio.run(api.handle_response_done({"type": "response.done", "response": {"id": "resp_tool_final_wrong_lineage"}}))
    ...
    assert kwargs["turn_id"] == "turn_3"
    assert kwargs["allow_cross_turn_rebind"] == ""
    parent_brief = api.get_continuity_brief("run-test", "turn_2", reason="post_wrong_lineage_tool_output")
    assert parent_brief.compound_request is not None
    assert any(step.kind == "report" and step.status == "pending" for step in parent_brief.compound_request.steps)
...
def test_handle_response_done_updates_turn_trace_to_complete_summary() -> None:
    api = _make_api()
        ...
        ("run-test", "turn_1"): SimpleNamespace(
            response_create_observation=SimpleNamespace(
                    context=SimpleNamespace(
                        ...
                        turn_id="turn_1",
                        canonical_key="turn_1::input_evt_1",
                        authority_retained_by="ai.response_create_arbitration.decide_response_create_arbitration",
                    ),
            ...
            warning_codes=(),
            authority_seams_seen=("ai.response_create_arbitration.decide_response_create_arbitration",),
    ...
    payload = None
    for call in debug_log.call_args_list:
        if call.args and call.args[0] == "decision_adapter_turn_trace payload=%s":
            payload = call.args[1]
    assert payload is not None
...
def test_handle_response_done_logs_info_turn_summary_for_complete_trace() -> None:
    api = _make_api()
    api._turn_arbitration_trace_by_key = {
        ("run-test", "turn_1"): SimpleNamespace(
            response_create_observation=SimpleNamespace(
    ...
    with patch("ai.realtime.response_terminal_handlers.logger.info") as info_log, patch("ai.realtime.response_terminal_handlers.logger.debug"):
        asyncio.run(api.handle_response_done({"type": "response.done", "response": {"id": "resp_1"}}))
    ...
    assert summary_call.args[1] == "run-test"
    assert summary_call.args[2] == "turn_1"
    assert summary_call.args[3] == "coherent"


def test_handle_response_done_logs_turn_review_summary() -> None:
    api = _make_api()
    api._turn_arbitration_trace_by_key = {
        ("run-test", "turn_1"): SimpleNamespace(
            response_create_observation=SimpleNamespace(
    ...
    assert payload is not None
    assert payload["trace_complete"] is True
    assert payload["terminal_summary"] == "terminal deliverable selected"
