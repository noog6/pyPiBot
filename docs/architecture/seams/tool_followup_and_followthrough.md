...
## Tool read-result capability authority

Read/query follow-up classification is sourced from `governance.tool_specs[*].capabilities` in `config/default.yaml` and normalized through `ai.tool_capabilities`. Runtime follow-up seams must ask that capability model whether a tool is read-only and user-result eligible instead of maintaining independent `read_`/`get_` allowlists. Registered tools must declare capabilities explicitly at startup; undeclared capabilities are conservative non-read/internal actions in non-validating helper contexts.

The response-create metadata construction seam assigns `read_purpose` when tool-followup metadata is built. Invocation context then refines the central capability declaration: direct user-answer reads become `user_requested_result`, compound/runtime-silent intermediates become `compound_intermediate` or `runtime_context`, and runtime/background-only reads do not open audible result obligations. Dual-purpose reads such as `recall_memories` are user-facing only when invoked as a direct user-requested follow-up; automatic memory/context retrieval must carry runtime context and remain `runtime_context`.
